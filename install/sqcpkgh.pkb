...
/* $Header: 215187.1 sqcpkgh.pkb 12.1.160429 2016/04/29 carlos.sierra mauro.pagano abel.macias@oracle.com $ */
  ...
  BEGIN
    ...
    BEGIN
      ...
      IF l_count > 0 THEN
        IF TO_NUMBER(sqlt$a.get_sqlt$_v$parameter2(s_sql_rec.statement_id, 'optimizer_dynamic_sampling')) BETWEEN 1 AND 3 THEN
          ...
          s_obs_rec.observation := 'Dynamic Sampling instance parameter is set to a small value of '||sqlt$a.get_sqlt$_v$parameter2(s_sql_rec.statement_id, 'optimizer_dynamic_sampling')||'.';
	...
	IF s_sql_rec.rdbms_release < 11 THEN
      ...
      THEN
        ...
        s_obs_rec.more        := 'SQLT$UDUMP "'||sqlt$a.get_udump_path||'" and USER_DUMP_DEST "'||sqlt$a.get_v$parameter('user_dump_dest')||'" should match for SQLT to find traces. Please fix executing connected as SYS: SQL> exec sys.sqlt$_trca$_dir_set';
        ins_obs;
      END IF;
      ...
      THEN
        ...
        s_obs_rec.more        := 'SQLT$BDUMP "'||sqlt$a.get_bdump_path||'" and BACKGROUND_DUMP_DEST "'||sqlt$a.get_v$parameter('background_dump_dest')||'" should match for SQLT to find traces. Please fix executing connected as SYS: SQL> exec sys.sqlt$_trca$_dir_set';
        ins_obs;
      END IF;
	ELSE
     ...
     THEN
        ...
        s_obs_rec.observation := 'SQLT$UDUMP and/or SQLT$BDUMP are not pointing to '||sqlt$a.get_diag_path;
        s_obs_rec.more        := 'SQLT$UDUMP "'||sqlt$a.get_udump_path||'" and SQLT$BDUMP "'||sqlt$a.get_bdump_path||'" should point to "'||sqlt$a.get_diag_path||'" for SQLT to find traces. Please fix executing connected as SYS: SQL> exec sys.sqlt$_trca$_dir_set';
        ins_obs;	  
	 END IF;
    END IF;
    ...
    IF s_sql_rec.rdbms_release >= 11 THEN
      ...
      BEGIN
        ...
        IF tsk_rec.status = 'ENABLED' AND l_count = 0 THEN
          ...
          s_obs_rec.observation := 'Automatic gathering of CBO statistics is enabled but no job was executed in the last 8 days';
    ...
    IF s_sql_rec.rdbms_release >= 11 THEN
      ...
      BEGIN
        ...
        IF tsk_rec.status = 'ENABLED' AND l_count <> 0 THEN
          ...
          s_obs_rec.object_type := 'DBMS_STATS';
          s_obs_rec.object_name := 'DBA_AUTOTASK_CLIENT_HISTORY';
          s_obs_rec.observation := 'Automatic gathering of CBO statistics is enabled but some job did not complete in the last 8 days';
    ...
    BEGIN
      ...
      IF l_count > 0 THEN
        ...
        s_obs_rec.observation := 'SQL Plan Baseline contains '||l_count||' non-reproduceable Plan(s).';
	...
	-- 160415 reword as a reminder
     BEGIN
      IF NVL(sqlt$a.get_sqlt$_v$parameter2(s_sql_rec.statement_id, 'shared_servers'),0) > 0 AND s_sql_rec.exadata = 'TRUE' THEN
        ...
        s_obs_rec.object_type   := 'OFFLOAD';
        s_obs_rec.object_name   := 'CELL OFFLOAD';
        s_obs_rec.observation   := 'Reminder: Offload is not used for SQLs executed from Shared Server.';
	...
	-- 160415 reword as a reminder
     BEGIN
      IF TRIM(UPPER(SUBSTR(LTRIM(s_sql_rec.sql_text),1,6))) IN ('INSERT','UPDATE','DELETE','MERGE') AND s_sql_rec.exadata = 'TRUE' THEN
        ...
        s_obs_rec.object_type   := 'OFFLOAD';
        s_obs_rec.object_name   := 'CELL OFFLOAD';
        s_obs_rec.observation   := 'Reminder: Offload is not used for SQLs that don''t use direct path reads.';
	...
	-- AutoDOP and no IO Calibration
     BEGIN
      IF UPPER(NVL(sqlt$a.get_sqlt$_v$parameter2(s_sql_rec.statement_id, 'parallel_degree_policy'),'NA')) IN ('AUTO','LIMITED') AND NVL(s_sql_rec.ioc_max_iops,0) = 0 THEN
        ...
        s_obs_rec.observation   := 'AutoDOP is enabled but there are no IO Calibration stats.';
  ...
  BEGIN
    ...
    IF p_tab_rec.num_rows = 0 AND (p_tab_rec.temporary = 'Y' OR p_tab_rec.table_name LIKE '%TEMP' OR p_tab_rec.table_name LIKE '%\_TEMP\_%' ESCAPE '\') THEN
      ...
      s_obs_rec.object_type := 'TABLE';
      s_obs_rec.object_name := p_tab_rec.owner||'.'||p_tab_rec.table_name;
      s_obs_rec.observation := 'TEMP table with zero num_rows but table was not empty! Statistics were gathered at the wrong time.';
      s_obs_rec.more      := 'Delete <a href="#tab_stats">table statistics</a> on this TEMP table.';
      ins_obs;

    END IF;

    -- check for Doc ID 1491748.1
    IF p_tab_rec.table_name = 'SEG$' AND p_tab_rec.owner = 'SYS'  THEN
      s_obs_rec.type_id     := E_TABLE;
      s_obs_rec.object_type := 'TABLE';
      s_obs_rec.object_name := p_tab_rec.owner||'.'||p_tab_rec.table_name;
      s_obs_rec.observation := 'Access to [DBA|ALL|USER]_SEGMENTS or [DBA|USER]_TS_QUOTES could be expensive under certain circumstances';
    ...
    BEGIN
      IF p_tab_rec.dv_censored = 'Y'  THEN
        ...
        s_obs_rec.object_type := 'TABLE';
        s_obs_rec.object_name := p_tab_rec.owner||'.'||p_tab_rec.table_name;
        s_obs_rec.observation := 'Table protected by Data Vault, low/high values will be missing from the report';
      ...
      BEGIN
        IF sqlt$a.get_sqlt_v$session_fix_control(p_tab_rec.statement_id, 13583722) > 0 AND p_tab_rec.partitioned = 'YES' THEN
          ...
          IF l_count > 0 THEN
            ...
            s_obs_rec.object_type := 'TABLE';
            s_obs_rec.object_name := p_tab_rec.owner||'.'||p_tab_rec.table_name;
            s_obs_rec.observation := 'Histograms from partitions may not be used for optimization.';
...
/*      BEGIN
      ...
      IF p_tab_rec.global_stats = 'NO' THEN
        ...
        IF p_tab_rec.partitioned = 'NO' THEN
          s_obs_rec.observation := 'CBO statistics might have been gathered using deprecated ANALYZE command.';
