...
/* $Header: 215187.1 sqcpkgd.pkb 12.1.160429 2016/04/29 carlos.sierra mauro.pagano abel.macias@oracle.com $ */
  ...
  BEGIN
    ...
    IF sqlt$a.get_rdbms_version >= '12.1.0.2' AND sqlt$a.get_param('c_inmemory') = 'Y' THEN 
      ...
      UPDATE sqlt$_dba_tab_cols a
         SET inmemory_compression = (SELECT inmemory_compression
                                       FROM sqlt$_gv$im_column_level b
                                      WHERE a.statement_id = p_statement_id
                                        AND a.statement_id = b.statement_id
                                        AND a.owner = b.owner
